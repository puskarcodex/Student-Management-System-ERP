...
import { Receipt, RefreshCw, Star, Wallet, Printer } from "lucide-react";
...
const handlePrint = (bill: FeeBill) => {
  ...
  const printContent = `
    ...
    <head>
      <meta charset="utf-8" />
      <title>Receipt — ${bill.studentName}</title>
      <style>
    ...
    <body>
      <div class="receipt">
        <div class="header">
          <h1>${bill.studentName}</h1>
          <p>${bill.className} &nbsp;·&nbsp; Invoice #${bill.id}</p>
        </div>

        <div class="meta">
          <div class="meta-tile">
            <div class="meta-label">Bill Date</div>
            <div class="meta-value">${bill.billDate}</div>
          </div>
          <div class="meta-tile">
            <div class="meta-label">Due Date</div>
            <div class="meta-value">${bill.dueDate}</div>
          </div>
          <div class="meta-tile">
            <div class="meta-label">Total Billed</div>
            <div class="meta-value">Rs. ${bill.totalAmount.toLocaleString()}</div>
          </div>
          <div class="meta-tile">
            <div class="meta-label">Amount Paid</div>
            <div class="meta-value" style="color:#059669;">Rs. ${bill.paidAmount.toLocaleString()}</div>
          </div>
        </div>

        ${bill.feeItems.length > 0 ? `
        ...
        <div class="totals">
          <div class="total-row">
            ...
            <span class="value">Rs. ${bill.totalAmount.toLocaleString()}</span>
          </div>
          <div class="total-row">
            <span class="label">Paid</span>
            <span class="value" style="color:#059669;">Rs. ${bill.paidAmount.toLocaleString()}</span>
          </div>
          <div class="total-row grand">
            <span class="label">Balance Due</span>
            <span class="value">Rs. ${bill.balanceAmount.toLocaleString()}</span>
          </div>
        </div>
...
export default function RecordDetails({
  ...
  const onSubmit = (data: FormData) => {
    ...
    console.log("Record Payment:", { billId: bill.id, newPaidAmount, newBalance, newStatus });
        ...
        <SheetHeader className="p-8 bg-primary/5">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-xs font-black uppercase tracking-widest text-muted-foreground/50 mt-0.5">
                {bill.className} · Bill #{bill.id}
              </p>
            </div>
          </div>
          <SheetDescription className="text-muted-foreground font-medium">
            View invoice details and record a payment
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className={`flex items-center justify-between p-4 rounded-2xl ${
              bill.balanceAmount > 0 ? "bg-rose-500/5" : "bg-emerald-500/5"
            }`}>
                  ...
                  {recurringItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between py-2 border-b border-border/40">
                      <span className="text-sm font-medium text-muted-foreground">
                        {item.feeHead}
                        {item.frequency && (
                          <span className="text-[10px] text-muted-foreground/40 ml-1">({item.frequency})</span>
                        )}
                      </span>
                  ...
                  {Number(watchedAmount) > 0 && (
                    <div className="space-y-2 pt-2 border-t border-border/40">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className={`font-black ${newBalance > 0 ? "text-rose-600" : "text-emerald-600"}`}>
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className={`text-xs font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
                          newStatus === "Paid" ? "bg-emerald-500/10 text-emerald-600" : "bg-blue-500/10 text-blue-600"
                        }`}>
                          {newStatus}
                        </span>
                      </div>
