...
import { Plus, Trash2, Receipt, RefreshCw, Star, Loader2 } from "lucide-react";
...
import type { FeeBill, FeeStructure, Student } from "@/lib/types";
import { feeBillsApi, studentsApi } from "@/lib/api";
...
const schema = yup.object({
  ...
  studentId:   yup.number().transform((v) => (isNaN(v) ? undefined : v)).required("Required"),
  dueDate:     yup.string().required("Due date is required"),
  paidAmount:  yup.number().transform((v) => (isNaN(v) ? 0 : v)).min(0).default(0),
  status:      yup.string().oneOf(["Paid", "Partial", "Pending", "Overdue"]).required(),
  feeItems:    yup.array().of(yup.object({
    feeHead:   yup.string().required("Required"),
    amount:    yup.number().transform((v) => (isNaN(v) ? 0 : v)).required().min(0),
...
export default function ManageFeeDetails({
  ...
  const {
    ...
    defaultValues: {
      ...
      feeItems: [{ feeHead: "", amount: 0, feeType: "Recurring", frequency: "Monthly" }],
    },
  });
  ...
  const filteredStudents = students.filter((s) => s.className === watchedClass);
  ...
  const handleClassChange = (classId: string) => {
    ...
    if (structure) {
      replace([
        ...structure.recurringItems.map((i) => ({ feeHead: i.feeHead, amount: i.amount, feeType: "Recurring" as const, frequency: i.frequency ?? "Monthly" })),
        ...structure.oneTimeItems.map((i) => ({ feeHead: i.feeHead, amount: i.amount, feeType: "One-Time" as const, frequency: undefined })),
      ]);
    } else {
      replace([{ feeHead: "", amount: 0, feeType: "Recurring", frequency: "Monthly" }]);
    }
  };
  ...
  useEffect(() => {
    ...
    if (bill) {
      reset({
        classId: bill.classId, studentName: bill.studentName, studentId: bill.studentId,
        dueDate: bill.dueDate, paidAmount: bill.paidAmount, status: bill.status,
        feeItems: (bill.feeItems ?? []).map((i) =>({
          feeHead: i.feeHead, amount: i.amount,
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      reset({ classId: "", studentName: "", studentId: 0 as any, dueDate: "", paidAmount: 0, status: "Pending", feeItems: [{ feeHead: "", amount: 0, feeType: "Recurring", frequency: "Monthly" }] });
    }
  }, [isOpen, bill, reset]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setSubmitError(null);
    try {
      const className = CLASS_OPTIONS.find((c) => c.value === data.classId)?.label ?? data.classId;
      const balanceAmount = Math.max(0, balance);

      if (bill) {
        // PUT /feebill/{id}
        await feeBillsApi.update(bill.id, {
          ...
          status:       data.status,
          feeItems:     data.feeItems as FeeBill["feeItems"],
          totalAmount,
          balanceAmount,
        });
      } else {
        // POST /feebill — backend CreateFeeRequest uses `amount` not `totalAmount`
        await feeBillsApi.create({
          studentId:    data.studentId,
          studentName:  data.studentName,
          classId:      data.classId,
          className,
          ...
          dueDate:      data.dueDate,
          amount:       totalAmount,       // backend field name
          totalAmount,
          paidAmount:   data.paidAmount,
          balanceAmount,
          status:       data.status,
      ...
      <SheetContent className="sm:max-w-lg border-none shadow-2xl p-0 flex flex-col">
        <SheetHeader className="p-8 bg-primary/5">
          ...
          <SheetDescription className="text-muted-foreground font-medium">
            {bill ? "Update the invoice details below" : "Select a class first — students and fees load automatically"}
          </SheetDescription>
        </SheetHeader>

        <div className="flex-1 overflow-y-auto px-8 py-6">
          <form id="fee-form" onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormSection title="Select Class & Student" icon={Receipt}>
              <FormField label="Class" error={errors.classId?.message}>
                ...
                />
                {watchedClass && structures.find((s) => s.classId === watchedClass) && (
                  <p className="text-[11px] font-bold text-primary ml-1">✓ Fee structure loaded automatically</p>
                )}
                {watchedClass && !structures.find((s) => s.classId === watchedClass) && (
                  <p className="text-[11px] font-bold text-rose-500 ml-1">⚠ No fee structure configured for this class</p>
                )}
              </FormField>
              ...
              <FormField label="Student" error={errors.studentName?.message}>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <Select value={field.value} onValueChange={handleStudentChange} disabled={!watchedClass}>
                      <SelectTrigger className="rounded-xl border-muted-foreground/20 disabled:opacity-50">
                        <SelectValue placeholder={!watchedClass ? "Select class first" : "Select student"} />
                      </SelectTrigger>
                      <SelectContent className="rounded-xl">
                        {filteredStudents.length === 0 ? (
                          <div className="px-4 py-3 text-xs text-muted-foreground font-bold">No students in this class</div>
                        ) : (
            ...
            <FormSection title="Fee Breakdown" icon={RefreshCw}>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => append({ feeHead: "", amount: 0, feeType: "Recurring", frequency: "Monthly" })}
                ...
                {fields.map((field, index) => {
                  if (watchedItems?.[index]?.feeType !== "Recurring") return null;
                  return (
                    <div key={field.id} className="flex gap-2 items-center">
                      <Controller name={`feeItems.${index}.feeHead` as "feeItems.0.feeHead"} control={control}
                        render={({ field: f }) => (
                          <Select value={f.value} onValueChange={f.onChange}>
                            <SelectTrigger className="rounded-xl flex-1 text-sm"><SelectValue placeholder="Fee Head" /></SelectTrigger>
                            <SelectContent className="rounded-xl">
                              {FEE_HEAD_OPTIONS.map((opt) => <SelectItem key={opt.value} value={opt.label}>{opt.label}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        )}
                      />
                      <Controller name={`feeItems.${index}.frequency` as "feeItems.0.frequency"} control={control}
                        render={({ field: f }) => (
                          <Select value={f.value ?? "Monthly"} onValueChange={f.onChange}>
                            <SelectTrigger className="rounded-xl w-28 text-sm"><SelectValue /></SelectTrigger>
                            <SelectContent className="rounded-xl">
                      ...
                      />
                      <Input type="number" {...register(`feeItems.${index}.amount` as "feeItems.0.amount")} placeholder="Rs."
                      ...
                      <button type="button" onClick={() => remove(index)} className="p-2 rounded-xl text-rose-400 hover:bg-rose-50 hover:text-rose-600 transition-colors">
              ...
              <div className="space-y-2 mt-2">
                <div className="flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => append({ feeHead: "", amount: 0, feeType: "One-Time" })}
                    className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-1 hover:opacity-70 transition-opacity">
                    <Plus className="w-3 h-3" /> Add
                  </button>
                </div>
                {fields.map((field, index) => {
                  if (watchedItems?.[index]?.feeType !== "One-Time") return null;
                  return (
                    <div key={field.id} className="flex gap-2 items-center">
                      <Controller name={`feeItems.${index}.feeHead` as "feeItems.0.feeHead"} control={control}
                        render={({ field: f }) => (
            ...
            <FormSection title="Payment Summary" icon={Star}>
              <div className="bg-muted/40 rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black uppercase tracking-widest text-muted-foreground/50">Grand Total</span>
                  <span className="text-2xl font-black text-foreground">Rs. {totalAmount.toLocaleString()}</span>
                </div>
                <FormField label="Amount Paid Now" error={undefined}>
                  <Input type="number" {...register("paidAmount")} placeholder="0"
                        ...
                        <SelectContent className="rounded-xl">
                          {(["Pending", "Partial", "Paid", "Overdue"] as const).map((s) => (
                            <SelectItem key={s} value={s}
                              className={s === "Paid" ? "text-emerald-600 font-bold" : s === "Overdue" ? "text-rose-600 font-bold" : s === "Partial" ? "text-blue-600 font-bold" : "text-amber-600 font-bold"}>
                              {s}
                            </SelectItem>
                          ))}
                        </SelectContent>
