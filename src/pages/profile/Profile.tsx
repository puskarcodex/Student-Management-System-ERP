...
import { Camera, User, Lock, ShieldCheck, LogOut } from "lucide-react";
...
export default function ProfilePage() {
  ...
  // Change password (mock — backend will handle real validation)
  const onPasswordSubmit = (data: PasswordFormData) => {
    setPasswordError("");
    // Mock: pretend current password is always "password" for now
    if (data.currentPassword !== "password") {
      setPasswordError("Current password is incorrect.");
      return;
    }
    // In real app: call PATCH /auth/change-password with data
      ...
      <div className="max-w-2xl mx-auto space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="text-center sm:text-left">
                ...
                <div className="flex items-center justify-center sm:justify-start gap-2 mt-3">
                  ...
                  <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-600">
                    Active
                  </span>
                </div>
              </div>
        ...
        <ProfileSection title="Personal Information" icon={User}>
          <form onSubmit={handleInfoSubmit(onInfoSubmit)} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              {infoSuccess && (
                <p className="text-[11px] font-bold text-emerald-600">✓ Profile updated successfully</p>
              )}
              <Button type="submit" className="rounded-xl bg-primary px-6 font-black shadow-lg shadow-primary/20">
                Save Changes
              </Button>
            </div>
          </form>
        ...
        <ProfileSection title="Change Password" icon={Lock}>
          <form onSubmit={handlePwdSubmit(onPasswordSubmit)} className="space-y-4">
            ...
            <p className="text-[11px] font-bold text-muted-foreground/50 ml-1">
              💡 Mock password is <span className="text-foreground/60">password</span> — replace with real API when backend is ready
            </p>

            <div className="flex items-center justify-end gap-3 pt-2">
              {passwordError && (
                <p className="text-[11px] font-bold text-rose-500">{passwordError}</p>
              )}
              {passwordSuccess && (
                <p className="text-[11px] font-bold text-emerald-600">✓ Password changed successfully</p>
              )}
              <Button type="submit" className="rounded-xl bg-primary px-6 font-black shadow-lg shadow-primary/20">
                Update Password
              </Button>
